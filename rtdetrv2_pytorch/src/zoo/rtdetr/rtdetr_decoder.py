...
class RTDETRTransformerv2(nn.Module):
    ...
    def _get_decoder_input(self,
        ...
        """
        디코더에 입력될 데이터를 준비하는 함수.
        
        Args:
            memory (torch.Tensor): 인코더의 출력을 나타내는 텐서, shape=[batch, num_tokens, hidden_dim]
            spatial_shapes (List): 피처맵의 각 레벨별 크기 정보
            denoising_logits (torch.Tensor, optional): 디노이징 학습을 위한 클래스 예측값
            denoising_bbox_unact (torch.Tensor, optional): 디노이징 학습을 위한 바운딩 박스 좌표

        Returns:
            content (torch.Tensor): 디코더의 입력 토큰
            enc_topk_bbox_unact (torch.Tensor): 선택된 바운딩 박스 좌표 (정규화되지 않음)
            enc_topk_bboxes_list (List[torch.Tensor]): 선택된 바운딩 박스 리스트
            enc_topk_logits_list (List[torch.Tensor]): 선택된 클래스 예측값 리스트
        """
        # 1. valid_mask 및 anchors 생성 (훈련 시 새로 생성, 평가 시 기존 사용)
        ...
        print(f"🔹 Before adjustment: Memory shape: {memory.shape}, Valid Mask shape: {valid_mask.shape}")
        
        # 2. valid_mask 크기가 memory와 일치하는지 확인하고 조정
        if valid_mask.shape[1] != memory.shape[1]:
            print(f"⚠️ Size mismatch detected! Adjusting valid_mask size... ({valid_mask.shape} → {memory.shape})")
            valid_mask = F.interpolate(valid_mask.float().unsqueeze(0), size=(memory.shape[1], 1), mode="nearest").squeeze(0).to(valid_mask.dtype)
        
        print(f"🔹 Adjusted valid_mask shape: {valid_mask.shape}, Memory shape: {memory.shape}")
        
        # 3. valid_mask를 memory에 적용하여 유효한 영역만 유지
        memory = valid_mask.to(memory.dtype) * memory  

        # 4. 인코더의 출력을 변환하여 디코더 입력 생성
        output_memory: torch.Tensor = self.enc_output(memory)
        enc_outputs_logits: torch.Tensor = self.enc_score_head(output_memory)
        enc_outputs_coord_unact: torch.Tensor = self.enc_bbox_head(output_memory) + anchors

        # 5. 최상위(top-k) 바운딩 박스 및 예측값 선택
        ...
        # 7. 디노이징 학습이 활성화된 경우 추가적으로 바운딩 박스 및 클래스 정보를 결합
